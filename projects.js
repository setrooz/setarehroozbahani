...
function initProjectsSphere() {
  ...
  function startAllOverlayVideos() {
    Object.values(
      loadedOverlayVideos
    ).forEach(video => {
      video.play().catch(() => {
  ...
  PROJECTS.forEach(
    (project, index) => {
      ...
      link.innerHTML =
        `<span class="nav-num">${project.num}</span>`;
  ...
  // ── Shared image/video canvas drawing ────────────────────────────────────
  ...
  function drawCardVisual(
    ...
    if (emphasized) {
      drawContext.shadowColor =
        'rgba(200,120,130,0.22)';
    ...
    // Video overlay, or static overlay for other projects
    const overlayVideo =
      loadedOverlayVideos[
        project.num
      ];
  ...
  function drawAtmosphere(
    ...
    if (W > 780) {
      ...
      ctx.strokeStyle =
        'rgba(200,120,130,0.42)';

      ctx.fillStyle =
        'rgba(200,120,130,0.55)';
  ...
  function showOverlay(card) {
    ...
    redrawSelectedOverlay();
  }

  function redrawSelectedOverlay() {
    if (
      !selectedCard ||
      !overlay.classList.contains(
        'visible'
  ...
  function animationLoop(time) {
    ...
    // Redraw the preview so the MP4 continues playing there.
    redrawSelectedOverlay();
  }
